...
export default function PerfilPage({ onBack }) {
  ...
  const { isDark, toggleTheme, palette, setPalette, customColors, setCustomColors } = useTheme()
            ...
            <div className="pt-2 border-t border-slate-100 dark:border-slate-700">
              ...
              <div className="grid grid-cols-4 gap-2">
                ...
                <button
                  ...
                  className={`py-2 px-1 text-center rounded-xl border-2 transition-all ${palette === 'custom' ? 'border-primary bg-primary/5' : 'border-transparent bg-slate-50 dark:bg-slate-700 hover:bg-slate-100 dark:hover:bg-slate-600'}`}
                >
                  <div className="w-6 h-6 rounded-full mx-auto mb-1 bg-gradient-to-br from-pink-500 via-yellow-400 to-cyan-400"></div>
                  <span className={`text-[10px] font-semibold ${palette === 'custom' ? 'text-primary' : 'text-slate-500'}`}>Custom</span>
                </button>
              </div>

              {/* Seletores de cor — só aparecem na paleta personalizada */}
              {palette === 'custom' && (
                <div className="mt-3 pt-3 border-t border-slate-100 dark:border-slate-700 space-y-1">
                  ...
                  {[
                    { key: 'fundo', label: 'Fundo do app' },
                    { key: 'conta', label: 'Cor das contas / cards' },
                    { key: 'barra', label: 'Cor das barras (topo)' },
                    { key: 'fonte', label: 'Cor da fonte' },
                  ].map(({ key, label }) => (
                    <label
                      key={key}
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span
                          className="w-9 h-9 rounded-lg border-2 border-slate-200 dark:border-slate-600 shadow-sm relative overflow-hidden"
                  ...
                  <button
                    ...
                    onClick={() => setCustomColors({
                      fundo: '#F8FAFC', conta: '#FFFFFF', barra: '#1A3A5C', fonte: '#1E293B',
                    })}
                    className="w-full mt-2 py-2 text-xs font-medium text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300 transition-colors"
                  >
